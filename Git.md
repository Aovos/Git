...
> Remove a file from the staging area while keeping the file changes:
```bash
...
> Move the current branch back to a specific commit:
```bash
...
> Temporarily switch to a previous commit:
```bash
